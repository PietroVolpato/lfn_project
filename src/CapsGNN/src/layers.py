...
class PrimaryCapsuleLayer(torch.nn.Module):
    ...
    def __init__(self, args, number_of_features):
        ...
        self.num_units = args.num_primary_units
        self.units = []
        for i in range(self.num_units):
            unit = torch.nn.Conv2d(
                in_channels=2,  # Match input channels
                out_channels=args.gcn_filters,
                kernel_size=(args.gcn_layers, 1),
                stride=1,
                bias=True
            )
            self.add_module(f"unit_{i}", unit)
            self.units.append(unit)

    @staticmethod
    def squash(s, epsilon=1e-7):
        """
        Squash activations with numerical stability.
        """
    ...
    def forward(self, x):
        ...
        x = x.view(batch_size, channels, spatial[0], -1)  # reshape to [batch, channels, height, width]
