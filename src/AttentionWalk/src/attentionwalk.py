...
class AttentionWalkTrainer(object):
    ...
    def save_embedding(self):
        ...
        # Retrieve model factor matrices
        left = self.model.left_factors.detach().numpy()  # shape: [N, half_dim]
        right = self.model.right_factors.detach().numpy().T  # shape: [N, half_dim]
        
        # Ensure consistent dimensions with the graph's node count
        num_nodes = len(self.graph)

        # (Optional) Slice if there's an off-by-one or mismatch
        left = left[:num_nodes, :]
        ...
        # Concatenate indices, left factors, and right factors
        embedding = np.concatenate([indices, left, right], axis=1)

        # Build column names: 1 for "id" + dimensions for the entire embedding
